...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    const req = driver.request(
      ...
      (res) => {
        ...
        res.on("error", (err) => {
          ...
          fail(new FitConnectNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      ...
      timer = setTimeout(() => {
        const err = new FitConnectNetworkError(`Request timed out after ${timeoutMs}ms`);
        fail(err);
        req.destroy(err);
    ...
    req.on("error", (err) => {
      ...
      fail(err instanceof FitConnectNetworkError ? err : new FitConnectNetworkError(err.message, { cause: err }));
    });
