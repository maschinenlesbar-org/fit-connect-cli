...
import { FitConnectNetworkError, redactUrl } from "./errors.js";
...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    try {
      ...
      reject(new FitConnectNetworkError(`Invalid URL: ${redactUrl(request.url)}`));
      return;
    }
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new FitConnectNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${redactUrl(request.url)}`));
      return;
    }
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            reject(new FitConnectNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          ...
          reject(new FitConnectNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      req.setTimeout(request.timeoutMs, () => {
        req.destroy(new FitConnectNetworkError(`Request timed out after ${request.timeoutMs}ms`));
      });
    }

    req.on("error", (err) => {
      // A timeout destroy already passes a FitConnectNetworkError; don't double-wrap.
      reject(err instanceof FitConnectNetworkError ? err : new FitConnectNetworkError(err.message, { cause: err }));
    });
